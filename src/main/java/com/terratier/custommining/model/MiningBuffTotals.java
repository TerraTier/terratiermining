...
public record MiningBuffTotals(Map<String, Double> attributes, int sources, Map<BuffSourceType, MiningBuffTotals> sourceBreakdown) {
    public MiningBuffTotals(Map<String, Double> attributes, int sources) {
        this(attributes, sources, new HashMap<>());
    }

    public static MiningBuffTotals none() {
    ...
    public MiningBuffTotals add(String key, double value, boolean multiply) {
    ...
    public double getTotalAdd(String prefix) {
        double total = 0.0;
    ...
    public double getTotalMultiplier(String prefix) {
        double total = 1.0;
        for (Map.Entry<String, Double> entry : attributes.entrySet()) {
            if (entry.getKey().startsWith(prefix) && isMultiplier(entry.getKey())) {
                total *= entry.getValue();
    ...
    public MiningBuffTotals merge(MiningBuffTotals other) {
